...
namespace Harpia.PrefabBrush
{
    public class PrefabBrush : EditorWindow
    {
        ...
        public VisualElement placementSection;
        ...
        public void CreateGUI()
        {
            ...
            //Snapping
            PB_GridSnappingManager.Init(root);
            ...
            //Rotation
            rotationField = new RotationTypeElement(root, "rotation-type", "rotation-fixed", "rotation-max", "rotation-min", null);
            ...
            makeErasableToggle = root.Q<Toggle>("objects-erasable-toggle");
            addToClippingCheckToggle = root.Q<Toggle>("objects-clipping-test-toggle");
            ...
            _showBrushGuideLines.RegisterFocusEvents_PB();

            brushGuideLinesDistance = root.Q<FloatField>("guide-lines-distance");
            brushGuideLinesDistance.RegisterFocusEvents_PB();
        ...
        public void UpdateUITool()
        {
            ...
            PB_GridSnappingManager.UpdateUI();
            ...
            PrefabBrushObject.SetClippingSize(isInEraserMode ? PB_EraserManager.eraserRadiusSlider : clippingToleranceSlider);
        ...
        private void OnSceneGUIMultiple()
        {
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.Repaint:
                    ...
                    PB_GridSnappingManager.DrawGrid(lastHitInfo, false);
                    if (showBoundsToggle.value) PrefabBrushObject.DrawBoundsAll(lastHitInfo.point, boundsColorField.value);
        ...
        private void PB_OnSceneGuiFocusInteraction()
        {
            ...
            if (focus == PB_EraserManager.eraserRadiusSlider || focus == PB_AdvancedSettings.eraserColor)
            {
                RaycastHit hitInfo = PrefabBrushTool.GetCenterRay(sceneCamera, GetRaycastMode(), layerMaskField.value);
            ...
            if (focus == PB_GridSnappingManager.gridSnapToggle || focus == PB_GridSnappingManager.gridSnapValueField || focus == PB_AdvancedSettings.gridColor || focus == PB_GridSnappingManager.gridOffsetField)
            {
                RaycastHit hitInfo = PrefabBrushTool.GetCenterRay(sceneCamera, GetRaycastMode(), layerMaskField.value);
                PB_GridSnappingManager.DrawGrid(hitInfo, true);
                return;
            }
        ...
        private void OnKeyDownPrecisionMode()
        {
            ...
            else if (PB_ModularShortcuts.randomRotation.IsShortcut())
            {
                ...
                {
                    Vector3 randomRot = rotationField.GetValue(true);
        ...
        private void PaintDrag()
        {
            ...
            bool hitRandomPoint = PrefabBrushTool.RaycastToWorldPoint(sceneCamera, randomPointInsideDisc, layerMaskField.value, out RaycastHit hitInfoRandomPoint, GetRaycastMode());
        ...
        public static PrefabBrushObject RegisterObject(GameObject go)
        {
            ...
            brushObject.Init(PB_PhysicsSimulator.IsUsingPhysics(), erasable, clippingTest, PB_PhysicsSimulator.GetPhysicalLayer(brushObject.gameObject));
        ...
        public void LoadTemplate(PrefabBrushTemplate prefabBrushTemplate)
        {
            ...
            PB_EraserManager.eraserRadiusSlider.SetValueWithoutNotify(_lastLoadedTemplate.eraserSize);
            PB_EraserManager.deletePhysicsOnlyToggle.SetValueWithoutNotify(_lastLoadedTemplate.deletePhysicsOnly);
            PB_EraserManager.eraserMask.SetValueWithoutNotify(_lastLoadedTemplate.eraserMask);
            PB_EraserManager.eraserTagMask.SetValueWithoutNotify(_lastLoadedTemplate.eraserTagMask);
            PB_GridSnappingManager.gridSnapValueField.SetValueWithoutNotify(_lastLoadedTemplate.gridSnapValue);
            ...
            PB_FakePlaneManger.fakePlaneNoRaycastToggle.SetValueWithoutNotify(_lastLoadedTemplate.useFakePlaneIfNoHit);
            PB_FakePlaneManger.fakePlaneYPosField.SetValueWithoutNotify(_lastLoadedTemplate.fakePlaneYPosition);
            ...
            PB_PhysicsSimulator.useCustomLayerToggle.SetValueWithoutNotify(_lastLoadedTemplate.physicsUseCustomLayer);
            PB_PhysicsSimulator.customLayerField.SetValueWithoutNotify(_lastLoadedTemplate.physicsCustomLayer);
            PB_PhysicsSimulator.UpdateUIPhysics();
            ...
            makeErasableToggle.SetValueWithoutNotify(_lastLoadedTemplate.makeErasable);
            addToClippingCheckToggle.SetValueWithoutNotify(_lastLoadedTemplate.addToClippingTest);

            angleLimitsField.value = _lastLoadedTemplate.angleLimits;

            PB_GridSnappingManager.LoadTemplate(_lastLoadedTemplate);
    ...
    public static class PrefabBrushTool
    {
        ...
        public static bool RaycastToWorldPoint(Camera c, Vector3 refPoint, int layerMask, out RaycastHit hit, PrefabBrush.RaycastMode mode)
        {
            ...
            {
                bool meshModeHit = PB_MeshRaycaster.Raycast(ray, PB_MeshBatcher.Batch(), Matrix4x4.identity, out MeshRaycastResult result);
        ...
        {
            if (!PrefabBrush.instance._showBrushGuideLines.value) return;
            Handles.zTest = CompareFunction.LessEqual;
            float distance = PrefabBrush.instance.brushGuideLinesDistance.value;
            float size2 = 3f;
            
            Vector3 drawPoint = PB_GridSnappingManager.TryToGetPositionOnGrid(hitInfo.point);
        ...
        public static RaycastHit GetCenterRay(Camera cam, PrefabBrush.RaycastMode mode, int mask)
        {
            ...
            {
                normal = Vector3.up,
                point = camPosition.y < 0 ? camPosition + camForward * 4f : PB_FakePlaneManger.RaycastIntoY0(ray),
            };
        ...
        {
            return new Vector3(
                desiredScale.x / parentLossyScale.x,
                desiredScale.y / parentLossyScale.y,
                desiredScale.z / parentLossyScale.z);
        }
    ...
    public static class PB_EraserManager
    {
        ...
        public static VisualElement _eraseModePanel;

        public static void Init(VisualElement root)
        {
            _eraseModePanel = root.Q<VisualElement>("erase-mode");

            deletePhysicsOnlyToggle = root.Q<Toggle>("erase-physics-only-toggle");
        ...
        {
            _currentEraserTagsSelected = new List<string>();
            int newValue = evt.newValue;

            //Everything
            if (newValue == -1)
            {
                _currentEraserTagsSelected = eraserTagMask.choices;
                return;
            }
            ...
            {
                char c = binary[i];
                if (c == '1') _currentEraserTagsSelected.Add(eraserTagMask.choices[binary.Length - i - 1]);
            }
        ...
        public static void DrawCircleEraser(RaycastHit hitInfo, float radius)
        {
            ...
            {
                float d = eraserRadiusSlider.value * 5;
                float eraserRadius = eraserRadiusSlider.value;
    ...
    public static class PB_PrecisionModeManager
    {
        ...
        public static void DrawTemporaryPrefab(RaycastHit hitPoint, Vector3 size = default, bool lerp = true)
        {
            ...
            float lerpSpeed = PB_GridSnappingManager.gridSnapToggle.value ? 40 : 20;
            finalRot = Quaternion.Lerp(tempGo.transform.rotation, finalRot, deltaTime * lerpSpeed);

            Vector3 hitPointPos = PB_GridSnappingManager.TryToGetPositionOnGrid(hitPoint.point);
            Vector3 pos;
            // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
            if (lerp)
                pos = Vector3.Lerp(tempGo.transform.position, hitPointPos + offset, deltaTime * lerpSpeed);
            else
                pos = hitPointPos + offset;
        ...
        private static Quaternion CalculateFinalRot(RaycastHit hitPoint, PrefabAttributes props)
        {
            ...
            Vector3 rotVec3Field = props.useCustomRotationMode ? props.GetRotation() : rotationField.GetValue(false);
            if (rotationField.GetMode() == PrefabBrush.RotationTypes.Scene_Camera_Rotation) rotVec3Field += currentRotationCamera;
            rotVec3Field += currentRotation;

            float multiplier = rotationType == PrefabBrush.RotationTypes.Scene_Camera_Rotation ? 2 : .5f;
            Quaternion defaultEuler = rotationType == PrefabBrush.RotationTypes.Scene_Camera_Rotation ? Quaternion.Euler(rotVec3Field) : Quaternion.identity;
            Quaternion finalRot = alignWithNormal ? PrefabBrush.GetAlignedRotation(hitPoint) : defaultEuler;
        ...
        public static void SetPrefabToPaint(PbPrefabUI ui)
        {
            ...
            {
                name = "Prefab Brush Preview Object",
        ...
        public static Vector3 AddToRotation(float value)
        {
            ...
            float rotationYValue = (currentRotation.y + value) % 360f;
            
            currentRotation.y = rotationMode == PrefabBrush.RotationTypes.Fixed ? PrefabBrushTool.RoundTo(rotationYValue, value) : rotationYValue;

            if (currentRotation.y < 0) currentRotation.y += 360;
            if (tempGo != null) DrawTemporaryPrefab(hit, default, false);

            //UI
            PrefabBrush.instance.rotationField.TrySetValue(PrefabBrush.RotationTypes.Fixed, currentRotation);
            
            return CalculateFinalRot(PrefabBrush.instance.lastHitInfo, customProps).eulerAngles;
        }
        ...
        public static void UpdateCurrentOffset() => currentOffset = PrefabBrush.instance.offsetField.GetValue(true);
        public static void UpdateCurrentScale() => currentScale = PrefabBrush.instance.scaleField.GetValue(true);

        public static void UpdateCurrentRotation()
        {
            if (PrefabBrush.instance.rotationField.GetMode() == PrefabBrush.RotationTypes.Scene_Camera_Rotation) return;
            currentRotationCamera = PrefabBrush.instance.rotationField.GetValue(true);
        }
    ...
    public static class PB_MultipleModeManager
    {
        ...
        public static void PaintPrefabMultiple(RaycastHit raycastHit, PbPrefabUI prefabToSpawn)
        {
            ...
            hitPoint = PB_GridSnappingManager.TryToGetPositionOnGrid(hitPoint);
            ...
            Vector3 offset = props.useCustomOffset ? props.GetOffset() : pb.offsetField.GetValue(true);

            PrefabBrush.PivotMode pivotMode = props.useCustomPivotMode ? props.pivotMode : pb.GetPivotMode();

            Vector3 rotVec3Field = props.useCustomRotationMode ? props.GetRotation() : pb.rotationField.GetValue(true);

            Quaternion rot = pb.toggleAlignWithGround.value ? PrefabBrush.GetAlignedRotation(raycastHit) : Quaternion.Euler(rotVec3Field);
            ...
            Vector3 scale = props.useCustomScale ? props.GetScale() : PrefabBrushTool.MultiplyVec3(pb.scaleField.GetValue(true), prefabToSpawn.prefabToPaint.transform.lossyScale);
    ...
    public static class PB_ShortcutManager
    {
        ...
        {
            if (ShortcutManager.instance.activeProfileId != shortcutProfileId) return;
            if (_previousProfile == shortcutProfileId || string.IsNullOrEmpty(_previousProfile)) _previousProfile = "Default";
    ...
    public static class PB_PhysicsSimulator
    {
        ...
        public static Vector3 GetImpulseForce() => impulseFieldVec3.useToggle.value ? impulseFieldVec3.GetValue(true) : Vector3.zero;
        ...
        public static void Init(VisualElement rootElement)
        {
            ...
            useCustomLayerToggle = rootElement.Q<Toggle>("physics-use-custom-toggle");
            useCustomLayerToggle.RegisterValueChangedCallback(OnUseCustomLayerToggle);

            customLayerField = rootElement.Q<LayerField>("physics-use-custom-layer");
        ...
        public static void UpdateRigidbodiesObjects()
        {
            ...
            {
                foreach (Rigidbody rigidbody in GetAllBodies()) RigidbodyData.AddRigidbody(rigidbody, false);
                UpdateUIPhysics();
                return;
            }
            ...
            {
                IEnumerable<Rigidbody> allRBs = GetAllBodies();
        ...
        public static void ApplyImpulse(GameObject go)
        {
            ...
            foreach (Rigidbody rb in rbs)
            {
                ...
                if (addImpulse) rb.AddForce(impulseFieldVec3.GetValue(true).normalized * force, ForceMode.Impulse);
            }
        }
        ...
        public static int GetPhysicalLayer(GameObject refGO)
        {
            ...
            //Lets check if the prefab has a custom layer
    ...
    public static class CustomPrefabProps
    {
        ...
        public static void Init(VisualElement root)
        {
            ...
            _rotationField = new RotationTypeElement(root, "custom-props-rotation-type", "custom-props-rotation-fixed",
                "custom-props-rotation-max", "custom-props-rotation-min", "custom-props-rotation-toggle");
        ...
        {
#if HARPIA_DEBUG
            Debug.Log($"[{nameof(CustomPrefabProps)}] Showing custom props for {obj.prefabToPaint.name}", obj.prefabToPaint);
#endif
    ...
    public static class PB_TerrainHandler
    {
        ...
        private static int GetTerrainAtPosition(Vector3 pos)
        {
            ...
            if (terrainCoordinate.x < 0 || terrainCoordinate.x >= splatMapData.Length) return -1;
            if (terrainCoordinate.z < 0 || terrainCoordinate.z >= splatMapData.GetLength(2)) return -1;
        ...
        private static Vector3Int ConvertToSplatMapCoordinate(Vector3 testPos)
        {
            Vector3 vecRet = new();

            Vector3 terrainPosition = lastTerrain.transform.position;

            float mapX = lastTerrainData.size.x * lastTerrainData.alphamapWidth;
            float mapZ = lastTerrainData.size.z * lastTerrainData.alphamapHeight;

            vecRet.x = (testPos.x - terrainPosition.x) / mapX;
            vecRet.z = (testPos.z - terrainPosition.z) / mapZ;

            return new Vector3Int((int)vecRet.x, 0, (int)vecRet.z);
        }
    ...
    public static class PB_UndoManager
    {
        ...
        {
#if HARPIA_DEBUG
            Debug.LogError("Finish this method");
#endif
            return;
        ...
        public static void PerformUndo()
        {
            ...
            PrefabBrush.PaintMode currentMode = PrefabBrush.instance.GetPaintMode();
    ...
    public abstract class PB_Vector3Element
    {
        ...
        protected void Init(VisualElement root, string enumFieldParam, string fixedFieldParam, string maxFieldParam, string minFieldParam, string useToggleParam)
        {
            enumField = root.Q<EnumField>(enumFieldParam);
            fixedField = root.Q<Vector3Field>(fixedFieldParam);
            maxField = root.Q<Vector3Field>(maxFieldParam);
            minField = root.Q<Vector3Field>(minFieldParam);
            ...
            {
                useToggle = root.Q<Toggle>(useToggleParam);
                useToggle.RegisterValueChangedCallback(OnUseToggleValueChanged);
            }
        }

        public abstract Vector3 GetValue(bool canGenerateRandom);
    ...
    {
        public RotationTypeElement(VisualElement root, string enumField, string fixedField, string maxField, string minField, string toggle)
        {
            Init(root, enumField, fixedField, maxField, minField, toggle);
        ...
        private void OnEnumValueChanged(ChangeEvent<Enum> evt)
        {
#if HARPIA_DEBUG
            //log
            //Debug.Log($"[{nameof(RotationTypeElement)}] Changed enum");
#endif
            ...
            if (modeInt == (int)PrefabBrush.RotationTypes.Scene_Camera_Rotation)
            {
                PB_PrecisionModeManager.UpdateCurrentRotation(Vector3.zero);
            }
        }

        public override Vector3 GetValue(bool canGenerateRandom)
        {
    ...
    public class Vector3ModeElement : PB_Vector3Element
    {
        ...
        {
            Init(root, enumField, fixedField, maxField, minField, useToggle);
    ...
    public static class PB_AdvancedSettings
    {
        ...
        public static void Init(VisualElement root)
        {
            ...
            maxEraserRadius.value = PB_EraserManager.eraserRadiusSlider.highValue;
            ...
            RegisterEvent(maxEraserRadius, PB_EraserManager.eraserRadiusSlider);
        ...
        public static void SaveValues()
        {
            ...
            if (PrefabBrush.instance.brushRadiusSlider != null) EditorPrefs.SetFloat("pb_max_brush_radius", PrefabBrush.instance.brushRadiusSlider.highValue);
            if (PB_EraserManager.eraserRadiusSlider != null) EditorPrefs.SetFloat("pb_max_eraser_radius", PB_EraserManager.eraserRadiusSlider.highValue);
            if (PB_PhysicsSimulator.impulseForceSlider != null) EditorPrefs.SetFloat("pb_max_impulse", PB_PhysicsSimulator.impulseForceSlider.highValue);
        }
    ...
    {
        private enum PenPressureUseMode
        {
    ...
    public static class PB_GridSnappingManager
    {
        public static Toggle gridSnapToggle;
        public static FloatField gridSnapValueField;
        public static Vector2Field gridOffsetField;

        private static Vector3 _gridPos;
        private static Color GridColor => PB_AdvancedSettings.gridColor.value;
        ...
        private static void OnGridSnapToggleChanged(ChangeEvent<bool> evt)
        {
            PrefabBrush.instance.UpdateUITool();
        }
        ...
        public static void DrawGrid(RaycastHit hitInfo, bool forceDraw)
        {
            switch (forceDraw)
            {
                ...
                case false when !gridSnapToggle.value:
                    return;
            }
            ...
            _gridPos = new Vector3(
                PrefabBrushTool.RoundTo(hitInfo.point.x, snapValue),
                hitInfo.point.y,
                PrefabBrushTool.RoundTo(hitInfo.point.z, snapValue)) + GetGridOffset();
        ...
        {
#if HARPIA_DEBUG
            Debug.Log($"[{nameof(PB_GridSnappingManager)}] Grid offset changed");
#endif
        ...
        public static void UpdateUI()
        {
            //harpia log
#if HARPIA_DEBUG
            Debug.Log($"[{nameof(PB_GridSnappingManager)}] UpdateUI");
#endif

            bool canUseGrid = CanUseGrid();
            bool isUsingGrid = gridSnapToggle.value;

            //UI
            gridSnapToggle.SetActive(canUseGrid);
            gridSnapValueField.SetActive(canUseGrid && isUsingGrid);
            gridOffsetField.SetActive(canUseGrid && isUsingGrid);
        ...
        {
            gridSnapToggle.value = lastLoadedTemplate.snapToGrid;
        ...
        public static Vector3 TryToGetPositionOnGrid(Vector3 refPos)
        {
            ...
            if (!gridSnapToggle.value) return refPos;
            ...
            Vector3 roundValue =
                new Vector3(
                    PrefabBrushTool.RoundTo(refPos.x, snapValue),
                    refPos.y,
                    PrefabBrushTool.RoundTo(refPos.z, snapValue));

            Vector3[] pos = new Vector3[4];
            pos[0] = roundValue + new Vector3(offset.x, 0, offset.z);
            pos[1] = roundValue + new Vector3(-offset.x, 0, offset.z);
            pos[2] = roundValue + new Vector3(offset.x, 0, -offset.z);
            pos[3] = roundValue + new Vector3(-offset.x, 0, -offset.z);
            ...
            {
                float cd = Vector3.SqrMagnitude(refPos - pos[i]);
                if (cd > d) continue;
                d = cd;
                index = i;
            }
    ...
    public static class PB_FakePlaneManger
    {
        ...
        public static void Init(VisualElement root)
        {
            fakePlaneNoRaycastToggle = root.Q<Toggle>("no-hit-y-pos-toggle");
            fakePlaneYPosField = root.Q<FloatField>("fake-plane-y-pos");
        ...
        public static void DrawFakePlane()
        {
            ...
            Vector3[] rectVertices = new[]
            {
                new Vector3(center.x - size, yPos, center.z - size),
                new Vector3(center.x + size, yPos, center.z - size),
                new Vector3(center.x + size, yPos, center.z + size),
                new Vector3(center.x - size, yPos, center.z + size)
            };
    ...
    public static class PB_ThumbnailGenerator
    {
        ...
        public static void GenerateThumbnail(PbPrefabUI ui)
        {
            ...
            loadQueue.Enqueue(new ThumbnailRequestModel
            {
                go = go,
                ui = ui
            });
